...
///
/// **TB-009:** Format-agnostic tokenizer loading for studio/pipeline use
///
/// Supports:
/// - HuggingFace tokenizer.json (Llama, Phi, Gemma, etc.)
/// - TinyBrain simple JSON format
/// - SentencePiece .model files (future)
/// - TikToken format (future)
///
...
public enum TokenizerFormat: Equatable {
    ...
    case sentencePiece // .model binary file (future)
    case tiktoken      // OpenAI format (future)
    
    /// Detect tokenizer format by inspecting file
    public static func detect(at path: String) -> TokenizerFormat? {
        ...
        // For JSON files, inspect content
        guard ext == "json",
...
public enum TokenizerLoader {
    ...
    public static func load(from path: String) throws -> any Tokenizer {
        ...
        case .sentencePiece:
            throw TokenizerError.unsupportedFormat("SentencePiece support coming in TB-010")
        
        case .tiktoken:
            throw TokenizerError.unsupportedFormat("TikToken support coming in TB-010")
        }
    }
    ...
    ///
    /// Search strategy:
    /// 1. Look for tokenizer.json in Models/tinyllama-raw/
    /// 2. Look for any tokenizer in Models/
    /// 3. Fallback to test vocab (demo mode)
    ///
    /// - Returns: Tokenizer instance (always succeeds with fallback)
    public static func loadBestAvailable() -> any Tokenizer {
        // Priority 1: TinyLlama tokenizer
        ...
        // Priority 2: Any tokenizer in Models/
        let modelsPath = resolvePath("Models") ?? "Models"
        if let files = try? FileManager.default.contentsOfDirectory(atPath: modelsPath) {
            for file in files where file.hasSuffix(".json") && file.contains("tokenizer") {
        ...
        // Priority 3: Test vocab (fallback for demos)
        print("ℹ️ No tokenizer found, using test vocabulary")
